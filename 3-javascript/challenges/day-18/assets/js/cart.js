    ...
    if(cartData.length > 0) {
        ...
        cartData.map((cartItem, cartIndex) => {
            ...
            shoppingCartItemTrElt.innerHTML = `
                <th>${cartIndex + 1}</th>
                <td><span data-cart-item-action="removeElt" class="bi bi-trash text-danger"></span></td>
                <td><img src="${cartItem.img}" class="img-thumbnail rounded" style="width:  80px;" alt=""></td>
                <td>${cartItem.name}</td>
                ...
                <td class="text-end">
                    <div class="d-flex gap-3 justify-content-end align-content-center">
                        <button class="btn btn-outline-secondary btn-sm py-0">
                            <span data-cart-item-action="minusQte" class="bi bi-dash"></span>
                        </button>
                        <div id="${'cartItemQuantityId'+ cartItem.id}"></div>
                        <button class="btn btn-outline-secondary btn-sm py-0">
                            <span data-cart-item-action="plusQte" class="bi bi-plus"></span>
                        </button>
                    </div>
                </td>
        ...
        shoppingCartEmptyTrElt.innerHTML = `<td colspan="7" class="text-center fw-bold">Votre panier est vide</td>`;
        shoppingCartListContainer.appendChild(shoppingCartEmptyTrElt);
    }
}

// exécution de la fonction d'affichage du panier
...
function updateShoppingCartElements(event) {
    ...
    if(dataCartItemIdElement && dataCartItemActionElement) {
        ...
        const dataCartItemIdValue = dataCartItemIdElement.getAttribute('data-cart-item-id');
        ...
        if(dataCartItemIdValue && dataCartItemActionValue) {
            ...
            if(['minusQte', 'plusQte'].includes(dataCartItemActionValue)) {
                ...
                helpers.adjustQuantity('cartItemQuantityId'+ dataCartItemIdValue, dataCartItemIdValue, actionValue);
                helpers.updateShoppingCartItemTotalPrice('cartItemTotalPriceId'+ dataCartItemIdValue, dataCartItemIdValue);
                helpers.updateShoppingCartTotalAmount('totalAmountId');  // mise à jour du montant total
            }

            // supprimer un article du panier
            if(dataCartItemActionValue === 'removeElt') {
                const removeConfirmation = confirm("Voulez-vous vraiment supprimer cet articel du panier !");
